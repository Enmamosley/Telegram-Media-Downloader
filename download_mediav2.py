...
def resolve_output_folder(msg, caption, grouped, base_output, structure="flat"):
    if structure == "flat":
        if grouped:
            if caption:
                folder = os.path.join(base_output, clean_filename(caption))
            else:
        ...
        if grouped:
            if caption:
                folder = os.path.join(base_output, ym, "captions", clean_filename(caption))
            else:
...
async def main():
    ...
    logging.info(f"Descargas paralelas activas: {args.concurrent}")
